...
\color{black}\bf
  Enhancing Earth Science Data Literacy Through Interactive Analysis of the Keeling Curve: A Case Study with Science-Gifted Students
  % 킬링 곡선의 상호작용적 분석을 통한 지구과학 데이터 리터러시 함양 : 과학영재학생들에게 적용
}
%the author(s)
{\color{black}\small
...
\begin{posterbox}[name=figures,column=1,below=Conclusion, above=bottom]{Enhance Data Literacy}
% 데이터 리터러시 함양
...
\footnotesize
    ...
    \vspace{0.5em}
    \footnotesize
    \textbf{Key Data Literacy Skills Developed}
    % 개발된 핵심 데이터 리터러시 기술
    \scriptsize
    \begin{enumerate}
...
% 결과
    ...
    \scriptsize
    \begin{itemize}
    \item successfully executed Python code
    % 파이썬 코드를 성공적으로 실행
    \item modified code to answer new questions
    % 새로운 질문에 답하기 위해 코드를 수정
    \item wrote original code for calculations
    % 계산을 위한 독창적인 코드를 작성
    \item understood DataFrame operations
    % DataFrame 연산을 이해
    \end{itemize}
