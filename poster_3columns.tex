...
This study presents a Python-based module using authentic atmospheric carbon dioxide (CO\textsubscript{2}) data from the Keeling Curve to build science data literacy in science-gifted high school students. 
% 본 연구는 킬링 곡선의 실제 대기 이산화탄소(CO2) 데이터를 활용한 파이썬 기반 모듈을 제시하여 과학영재 고등학생의 과학 데이터 리터러시를 함양한다.
...
Assessments of 36 12th-grade students indicated improved ability to interpret scientific data and reason about measurement uncertainty. \\
% 과학영재학교 3학년(12학년) 36명 대상 평가에서 학생들의 과학 데이터 해석 능력과 측정 불확실성에 대한 추론 능력이 향상된 것으로 나타났다.
\vspace{0.2em}
\textbf{Keywords:} climate change education; science data literacy; Keeling Curve; Python; science-gifted students.
% 핵심어: 기후변화 교육; 과학 데이터 리터러시; 킬링 곡선; 파이썬; 과학영재 학생.
\end{posterbox}
...
% 결과
    ...
    % 학생들은 파이썬 코드를 실행하고, 새로운 질문에 답하기 위해 코드를 수정하며, 계산을 위한 코드를 작성하고 DataFrame 연산을 적용하였다.
...
% 증거 기반: 학생들은 단순한 수사가 아닌 데이터에서 기후변화를 확인하게 된다.
\end{itemize}
...
% 학생들은 계산 도구를 사용하여 실제 과학 데이터셋과 성공적으로 상호작용한다.
...
% 이 접근법은 기후변화 교육이 수동적 정보 소비를 넘어 능동적 과학 탐구로 나아갈 수 있음을 보여준다.
...
% 킬링 곡선 데이터셋을 통해 데이터 리터러시를 함양함으로써, 학생들은 21세기 시민성에 필수적인 개념적 이해와 실용적 기술을 얻게 된다.
...
% 대기 CO2 데이터는 UC San Diego 소속 Scripps 해양학 연구소의 Scripps CO2 프로그램에서 제공되었다.
